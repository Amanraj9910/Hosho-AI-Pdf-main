// // ✅ Enhanced Chat Interface with context-aware RFP extraction
...
//         case 400: return "Invalid request format. The message structure may be incorrect.";
//         case 401: return "Authentication failed. API key may be invalid or expired.";
//         case 403: return "Access forbidden. Check API permissions.";
//         case 429: return "Rate limit exceeded. Please wait before trying again.";
//         case 500: return "Azure OpenAI service error. Please try again later.";
//         default: return `API request failed with status ${response.status}`;
//       }
//     }
...
//     try {
//       const fullText = extractedData?.content?.text || '';

//       const systemPrompt = `You are an intelligent AI assistant helping users interact with uploaded documents such as RFPs, SWIFT files, contracts, technical PDFs, and regulatory notices.

// Your job is to provide fact-based, well-structured answers strictly derived from the document's content. The full text of the document has been extracted and passed to you.

// ---
// DOCUMENT METADATA:
// Name: \${extractedData.metadata?.fileName || 'Unknown'}
// Page Count: \${extractedData.metadata?.pageCount || 'Unknown'}
...
// 🟦 GENERAL RULES:
// - ✅ Always extract information directly from the document.
// - ✅ If a section spans multiple pages, combine and present it in full.

// 🧩 STRUCTURED OUTPUT FORMATTING:
// - Use clearly labeled section titles and emoji tags (📘, 📅, 💰, 📌, etc.) to improve readability.
// - Use **headings** for major sections (e.g., "2.1 Size and Technology Requirements").
// - Use **lettered bullet points (a), b), c))** for subpoints under each heading.
// - Use **tables** for:
//   - 📅 Timelines (submission, licensing, notification)
//   - 💰 Performance bond values
//   - 🏢 Licensing & compliance checkpoints
// - Use bullet lists to break down complex requirements or eligibility criteria.
// - Respond in clean plain text or HTML-safe output for UI display (no markdown).

// 📘 RFP DOCUMENTS (Especially Section 2 - RFP Requirements):
// - If the user asks for “RFP Requirements” or mentions “Section 2”, extract the **entire Section 2** and structure it as:
//   - Section headers
//   - Subpoints in readable bullets
//   - Timeline tables
//   - Performance bond details
//   - Any other submission/compliance blocks
// - Format should be visually clear, skimmable, and factual.

// 📊 SWIFT FILE TAG EXTRACTION:
// - If the user asks for SWIFT tags, output a table using **pipe separators**:
//   Format:  
//   Tag | Heading | Data

//   Rules:
//   - Each tag must appear only once per row
//   - If repeated, label as 71F-1, 71F-2 or merge values
//   - Remove unnecessary characters (like #, /, newline artifacts)
//   - Keep headings standardized (e.g., "Sender's Reference", not "Sender Ref")
//   - Ensure table rows are **neatly aligned** and consistently formatted

// 🛑 FORMATTING RULES (GLOBAL):
// - ❌ Do NOT use markdown syntax (no *, **, -, etc.)
// - ✅ Use HTML-safe format where necessary (for chat UIs)
// - ✅ Use spacing and indentation for clean visual alignment
// - ❌ Avoid long unbroken paragraphs


// 🤖 PERSONALITY:
// - Keep responses professional, accurate, clear, and non-repetitive.
// - Do not include filler phrases like “Based on your request…” unless clarity requires it.

// Only respond with structured, well-extracted, document-grounded answers.
...
//       // Previously used a hard-coded api-key here; use environment variables instead.
...
//             >
//               <div
//                 className={`flex max-w-[85%] ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start space-x-2`}
//               >
//                 <div
